 ...
 * @link     https://www.magetrend.com/magento-2-pdf-invoice-pro
 */
class Table extends \Magetrend\PdfTemplates\Model\Adapter\TcPdf\Element
{
    ...
    public function drawHeaderBackground()
    {
        ...
        $width = $this->toPoint($attributes['table_width']);
        $height = $this->toPoint($this->getHeaderHeight());

        if (isset($attributes['table_header_background'])) {
            $fillColor = $this->getPdfColor($attributes['table_header_background']);
    ...
    public function writeHeaderText()
    {
        ...
        $this->setTextColor($attributes['table_header_font_color']);
    ...
    public function drawRowText($columnsData, $topY)
    {
        ...
        foreach ($columnConfig as $columnName => $column) {
            ...
            foreach ($columnData['text'] as $lineKey => $textRow) {
                ...
                $font = $this->getPdfFont($textRow['font']);

                $this->pdf->SetFont($font, '', $this->toPoint($fontSize));
                $this->setTextColor($textRow['color']);
